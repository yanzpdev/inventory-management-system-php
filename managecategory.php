  ...
  <body>
    <nav id="navbar" class="navbar fixed-top navbar-expand-lg navbar-light bg-light">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          ...
          <ul class="navbar-nav justify-content-end">
            ...
            <li class="nav-item">
                <button class="btn btn-success btn-sm mt-1 ml-2" data-toggle="modal" data-target="#addCategory"> Add Category </button>
            </li>
            <li class="nav-item">
    ...
    <div class="container-fluid mt-5">
      ...
      ?>
        ...
        <div class="col-md-7 ml-3 shadow p-3 card ">
           <h5 class="card-title text-primary text-center mt-2">Manage Categories</h5>
          <!---UPPER CARDS -->
          <table class="table table-hover">
          <caption class="text-muted ml-2"><small>All categories from this system are shown.</small></caption>
          <thead>
           <tr>
            <th>Category ID</th>
            <th>Category</th>
          ...
          <?php
            ...
            $pagefirst_res = ($page-1) * $limit;
             ...
             $sql = $conn->prepare("SELECT * FROM `category` ORDER BY(departmentname) ASC LIMIT ".$pagefirst_res.','.$limit);
            $sql->execute();

            while($fetch = $sql->fetch()){
          ?>
          <tr>
            <td><?php echo $fetch['categoryid']?></td>
            <td><?php echo $fetch['categoryname']?></td>
            <td><?php echo $fetch['departmentname']?></td>
            <td><button class="btn btn-warning btn-sm text-light" data-toggle="modal" data-target="#updatecategory<?php echo $fetch['categoryid']?>">&#9997;&#127995;</button> <a class="btn btn-danger btn-sm" id="btnDelete" href="admindeletecateg.php?id=<?php echo $fetch['categoryid']?>">&times;</a></td>
          </tr>
        ...
        <?php } ?>
          ...
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link" href="managecategory.php?page=<?= $prev; ?>" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
              </a>
            </li class="page-item">
            <?php for($i = 1; $i<= $page_num; $i++) : ?>
              <li><a class="page-link" href="managecategory.php?page=<?= $i; ?>"><?= $i; ?></a></li>
            <?php endfor; ?>
            <li class="page-item">
              <a class="page-link" href="managecategory.php?page=<?= $next; ?>" aria-label="Next">
    ...
    <!---- CREDENTIALS MODAL ---->
      ...
      <?php
            ...
            $sqldep = $conn->prepare("SELECT * FROM `inventory`");
            $sqldep->execute();
     ...
     <div class="modal fade" id="addCategory" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <form method="POST" action="managecategoryadd.php">
            <div class="modal-header">
            ...
            <div class="modal-body">
              ...
              <div class="modal-footer">
                 <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
                <button type="submit" id="btnUpdate" name="updatecateg" class="btn btn-primary btn-sm">Add Category</button>
              </div>
              </div>
            </div>  
          </form>
      ...
      <?php
            ...
            while($fetch = $sql->fetch()){
        ?>
      <div class="modal fade show" id="updatecategory<?php echo $fetch['categoryid']?>" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <form method="POST" action="updatecategadmin.php">
            <div class="modal-header">
